...
public class DurabilityViewer {
    ...
    public DurabilityViewer(Minecraft mc) {
        MC = mc;
        if (ConfigurationHandler.showHotBarDurability) DrawHotBarOverlay(MC.thePlayer);
        if (ConfigurationHandler.showArmorDurability) DrawArmor(MC.thePlayer.getArmorInventoryList());
    }
    ...
    private void DrawHotBarOverlay(EntityPlayer player) {
        ...
        for (int SlotID = 0; SlotID <= 8; SlotID++) {
            ...
            // check if Tool
            if (HotBarItem != null && HotBarItem.getMaxDamage() > 0) {
